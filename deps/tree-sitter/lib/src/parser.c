...
static bool ts_parser__select_tree(TSParser *self, Subtree left, Subtree right) {
  ...
  printf("ERROR: ambiguity not resolved by precedence\n");
  printf("-----\n");
  printf("%s", ts_subtree_string(left, self->language, false));
  printf("\n-----\n");
  printf("%s", ts_subtree_string(right, self->language, false));
  printf("\n");
  exit(1);
